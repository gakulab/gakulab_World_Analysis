...
fun_Shannon <- function(data,TotalCatch_Tonnes,Total_Value){
  data %>% 
    ...
    mutate(Percentage_Catch = (TotalCatch_Tonnes/sum(TotalCatch_Tonnes)),
           Percentage_Value = (Total_Value/sum(Total_Value))) %>%
    mutate(Shannon_Catch = exp(-(Percentage_Catch*log(Percentage_Catch))),
           Shannon_Value = exp(-(Percentage_Value*log(Percentage_Value)))) %>%
    ungroup() %>% 
    group_by_(.dots = lazyeval::lazy_dots(Year)) %>% 
...
fun_CV <- function(data,Catch,Value){
  data %>%
    group_by(.dots = lazyeval::lazy_dots(Year,FishName_EN)) %>%
    summarise(Catch = sum(Catch)/1000,
...
#CV2
fun_CV_gaku <- function(data,Catch,Value){
  data %>%
    group_by(.dots = lazyeval::lazy_dots(Year)) %>%
    summarise(Catch = sum(Catch)/1000,
              Value = sum(Value)/1000000) %>%
